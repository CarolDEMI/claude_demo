...
CREATE TABLE cpz_qs_newuser_channel_i_d_new (
    ...
    is_returned_1_day REAL NOT NULL DEFAULT 0,     -- 次日留存用户数
    zizhu_revenue_1 REAL NOT NULL DEFAULT 0,       -- 首日收入（单位：元）
    zizhu_revenue_1_aftertax REAL NOT NULL DEFAULT 0, -- 首日收入税后（单位：元）
...
CREATE TABLE dwd_ttx_market_cash_cost_i_d_new (
    ...
    -- 指标字段
    cash_cost REAL NOT NULL DEFAULT 0,  -- 现金成本（单位：元）
...
CREATE TABLE daily_metrics_summary (
    ...
    retained_users REAL,                -- 留存用户数
    total_revenue REAL,                 -- 总收入（元）
