...
SELECT 
    ...
    ROUND(cash_cost, 2)  -- 保留2位小数，处理精度问题
...
SELECT 
    ...
    ROUND(SUM(a.zizhu_revenue_1), 2) as total_revenue,
    ROUND(COALESCE(SUM(b.cash_cost), 0), 2) as total_cost,
    ...
    ROUND(SUM(a.zizhu_revenue_1) / NULLIF(SUM(CASE WHEN a.status = 'good' AND a.verification_status = 'verified' THEN a.newuser ELSE 0 END), 0), 2) as arpu,
    ROUND(SUM(b.cash_cost) / NULLIF(SUM(CASE WHEN a.status = 'good' AND a.verification_status = 'verified' THEN a.newuser ELSE 0 END), 0), 2) as cpa
...
SELECT 
    ...
    SUM(a.newuser) as all_users,
    SUM(a.is_returned_1_day) as retained_users,
    ROUND(SUM(a.zizhu_revenue_1), 2) as revenue,
    ROUND(COALESCE(SUM(b.cash_cost), 0), 2) as cost,
    ROUND(SUM(CASE WHEN a.status = 'good' THEN a.newuser ELSE 0 END) * 100.0 / NULLIF(SUM(a.newuser), 0), 2) as good_rate,
    ROUND(SUM(a.is_returned_1_day) * 100.0 / NULLIF(SUM(CASE WHEN a.status = 'good' AND a.verification_status = 'verified' THEN a.newuser ELSE 0 END), 0), 2) as retention_rate,
    ROUND(SUM(a.zizhu_revenue_1) / NULLIF(SUM(CASE WHEN a.status = 'good' AND a.verification_status = 'verified' THEN a.newuser ELSE 0 END), 0), 2) as arpu,
    ROUND(SUM(b.cash_cost) / NULLIF(SUM(CASE WHEN a.status = 'good' AND a.verification_status = 'verified' THEN a.newuser ELSE 0 END), 0), 2) as cpa
